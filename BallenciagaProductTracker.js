...
(function() {
  ...
  const PRODUCT_SELECTOR = 'article, .product-card, [class*="product"]'; 
  ...
  /**
   * Create a unique ID for a product based on name and price
   ...
   */
  function createProductId(name, price) {
    return (name + '|' + price).toLowerCase();
  }
  ...
  function checkVisibleProducts() {
    ...
    products.forEach(function(product) {
      ...
      if (isVisible(product)) {
        ...
        if (name && price) {
          // Create unique ID
          const productId = createProductId(name, price);
